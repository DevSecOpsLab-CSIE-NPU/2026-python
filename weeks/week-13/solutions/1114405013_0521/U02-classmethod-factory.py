...
p4 = Point.from_tuple((3, 4))        # 從 tuple
p5 = Point.origin()                  # 工廠方法
...
cp2 = ColoredPoint.from_list([7, 8])
print(cp)            # ColoredPoint(5, 6, color='black')
print(cp2)           # ColoredPoint(7, 8, color='black')
print(type(cp))      # <class '__main__.ColoredPoint'>，不是 Point！
print(type(cp2))     # 也是 ColoredPoint
...
print("\n=== CPE：從字串建立成本表 ===")
flat = ' '.join(str(1) for _ in range(36))
parsed_table = CostTable.from_flat_string(flat)
print('parsed_table.total_cost(255, 10)=', parsed_table.total_cost(255, 10))
# 記憶重點 ──────────────────────────────────────────────────
# @classmethod 的第一個參數是 cls（class 本身），不是 self（物件）
# cls(...)  等於 ClassName(...)，但繼承時會自動用呼叫的子類
# 另一種相關的 decorators：@staticmethod 既沒有 cls 也沒有 self
# 常用於：替代構造器、工廠方法、從不同格式解析資料
# 只要 constructor 行為跟 __init__ 不同，就可以用 classmethod 分工
