...
# 原因：cart=[] 這個 list 在 def 時就建好了，三次呼叫都用同一個 list 物件
# 也就是說：def 只是定義函數時建立預設值，後續呼叫不會重新評估 cart=[]
...
print(add_to_cart_safe("葡萄"))  # ['葡萄']
# 小提醒：None 當作 sentinel 值很常見，因為 None 本身是不可變
...
# 原因：迴圈結束後 i=4，閉包裡的 lambda 去查 i 時，全都查到最後的值
# 這個現象也稱為「延遲綁定」（late binding），對迴圈中建立函數最容易出錯
...
# 這裡的 lambda i=i: i 會在定義 lambda 時就把 i 的當前值存到預設引數裡
...
print(c2(), c2())         # 11 12
print(c1())               # 4（c1 和 c2 是各自獨立的計數器）# 如果沒有 nonlocal，count += 1 會在 counter 內建新的區域變數，Python 會出錯

print("\n--- nonlocal 的另一種寫法：用 mutable 物件儲存狀態 ---")
def make_counter_list(start=0):
    count = [start]  # list 是可變物件
...
c3 = make_counter_list()
print(c3(), c3())  # 1 2
# 這種寫法不需要 nonlocal，但更常見的還是用 nonlocal 表達「我在修改外層變數」
# ── 實際應用：用閉包做「一次性」工具函數 ────────────────
