...
print("=== with 開檔：自動關閉 ===")
with open("week13_demo.txt", "w") as f:
    f.write("Hello from Week 13\n")

with open("week13_demo.txt", "r") as f:
    print(f.read().strip())
...
class Timer:
    ...
    def __enter__(self):
        ...
        print("⏱  開始計時")
        # __enter__ 的回傳值就是 with ... as [VAR] 中的 VAR
        return self   

    def __exit__(self, exc_type, exc_val, exc_tb):
        # exc_type, exc_val, exc_tb 用於捕捉區塊內發生的例外
        elapsed = time.time() - self.start
        print(f"⏱  結束：{elapsed:.4f} 秒")
        return False  # 回傳 False 表示若有例外發生，應繼續向外拋出
...
def section(title):
    ...
    print(f"  {title}")
    print(f"{'='*40}")
    # yield 之前相當於 __enter__，yield 之後相當於 __exit__
    yield           
    print(f"{'─'*40}")
