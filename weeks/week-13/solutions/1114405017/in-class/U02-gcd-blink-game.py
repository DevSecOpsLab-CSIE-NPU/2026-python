# ╔════════════════════════════════════════════════════════════════════════════╗
# ║           U02. GCD 閃燈觀察器（課堂互動遊戲 - 視覺化 LCM 概念）            ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
# 【程式目標】
# 透過 tkinter 建立一個互動式的視覺化工具，展示 LCM（最小公倍數）與 GCD（最大公因數）
# 的概念。4 盞不同顏色的燈，各有不同的閃爍週期（間隔），觀察它們何時同時亮起。
#
# 【核心邏輯】
# - 每盞燈有一個「間隔」大小，按節拍計數
# - 當 tick % interval == 0 時，該燈閃爍一次
# - 所有燈同時亮起的節拍數 = LCM（最小公倍數）
# - 所有間隔的最大公因子 = GCD（最大公因數）
#
# 【玩法】
#   1. 按「▶ 開始」讓燈自動閃爍
#   2. 觀察哪幾盞燈同時亮起 → 那個節拍數就是它們的 LCM
#   3. 調整 Spinbox 改變各燈的間隔，看 GCD 和 LCM 如何變化
#   4. 按「⏭ 一拍」逐拍手動推進，細看每一拍的亮燈狀態
#   5. 按「⏸ 暫停」 / 「↺ 重置」操作
#
# 【UI 元素】
# - Canvas：繪製 4 盞燈、進度環、倒數數字
# - Spinbox × 4：調整各燈的間隔（1–20 拍）
# - Scale：調整自動推進的速度（150–2000 ms/拍）
# - 按鈕：開始、一拍、暫停、重置
# - 狀態列：顯示節拍數、GCD、LCM、同步提示
...
def multi_gcd(nums):
    ...
    參數：
        nums (list)：整數列表，例如 [2, 3, 4, 6]
    
    返回：
        int：所有數字的最大公因子，例如 gcd(2,3,4,6) = 1
    
    原理：
        使用 functools.reduce 逐兩個數字計算 GCD
        reduce(gcd, [a, b, c, d]) → gcd(gcd(gcd(a, b), c), d)
    """
...
def multi_lcm(nums):
    ...
    返回：
        int：所有數字的最小公倍數，例如 lcm(2,3,4,6) = 12
    
    原理：
        使用公式：LCM(a, b) = a * b / GCD(a, b)
        逐兩個數字累積計算，最後得到所有數字的 LCM
    """
    result = nums[0]
    for n in nums[1:]:
