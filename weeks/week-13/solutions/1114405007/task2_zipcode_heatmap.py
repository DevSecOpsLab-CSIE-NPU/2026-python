...
ZIPCODE_TO_COUNTY = {
    ...
    "945": "屏東縣", "946": "屏東縣", "947": "屏東縣", "950": "台東縣", "951": "台東縣",
    "952": "台東縣", "953": "台東縣", "954": "屏東縣", "955": "屏東縣", "956": "屏東縣",
...
def get_top_counties(all_years: dict[int, dict[str, int]], top_n: int = 10) -> list[str]:
    """6 年合計，回傳人數前 top_n 的縣市。"""
    total: Counter[str] = Counter()
    for year_counts in all_years.values():
        total.update(year_counts)

    return [county for county, _ in total.most_common(top_n)]


def plot_county_heatmap(all_years: dict[int, dict[str, int]], counties: list[str], out_path: Path) -> None:
    """繪製「縣市 x 年份」熱力圖。"""
    matrix = np.array(
...
def main() -> None:
    ...
    out_path = Path(__file__).parent / "output" / "task2.png"
    plot_county_heatmap(all_years, counties, out_path)
