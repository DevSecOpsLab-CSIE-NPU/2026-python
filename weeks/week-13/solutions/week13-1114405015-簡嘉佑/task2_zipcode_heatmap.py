...
def plot_heatmap(all_years: dict[int, dict[str, int]], output_path: Path) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="繪製 109-114 學年度縣市熱力圖")
    parser.add_argument("--data-dir", type=Path, default=DATA_DIR)
    parser.add_argument("--output", type=Path, default=OUTPUT_DIR / "task2.png")
    args = parser.parse_args()

    all_years = {year: load_county_counts(year, args.data_dir) for year in [109, 110, 111, 112, 113, 114]}
    plot_heatmap(all_years, args.output)
