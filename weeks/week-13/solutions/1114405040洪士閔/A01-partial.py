...
# 此版本在原始程式基礎上增加更完整的中文註解，方便學習與教學使用。
from functools import partial

# ── 基本概念：固定部分參數，產生新函數 ───────────────────

def power(base, exp):
    # power：計算 base 的 exp 次方
    return base ** exp

# 用 partial 固定 exp 參數，產生新函數 square、cube
...
def get_score(student, subject):
    # get_score：依 subject 回傳學生的該科成績
    return student[subject]

# 用 partial 把 subject 固定，sorted 時直接把 partial 當作 key 傳入
...
def cost_in_base(n, base, costs):
    ...
    - n: 要計算成本的整數
    - base: 進位（例如 2~36）
    - costs: 長度至少為 base 的列表，每個索引代表該數字的成本
    回傳：該數字在此進位下的總成本
    """
    if n == 0:
        # 特別處理 0：直接回傳第 0 位的成本
...
# ── 固定 print 的格式 ─────────────────────────────────────
# 有時候需要把多個輸出印在同一行，partial 可以把 print 的參數預先固定
