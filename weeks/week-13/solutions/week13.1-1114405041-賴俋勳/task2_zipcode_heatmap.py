...
ZIPCODE_TO_COUNTY: dict[str, str] = {
    ...
    "730": "台南市", "731": "台南市", "732": "台南市", "733": "台南市", "734": "台南市", "735": "台南市", "736": "台南市",
    ...
    "950": "台東縣", "951": "台東縣", "952": "台東縣", "953": "台東縣", "954": "台東縣",
    "955": "台東縣", "956": "台東縣", "957": "台東縣", "958": "台東縣", "966": "台東縣",
    "970": "花蓮縣", "971": "花蓮縣", "972": "花蓮縣", "973": "花蓮縣", "974": "花蓮縣", "975": "花蓮縣",
    "976": "花蓮縣", "977": "花蓮縣", "978": "花蓮縣", "981": "花蓮縣", "983": "花蓮縣",
}
...
def load_county_counts(year: int, data_dir: Path) -> dict[str, int]:
    ...
    with open(filename, encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            county = zip_to_county(row.get("郵遞區號", "").strip())
...
def get_top_counties(all_years: dict[int, dict], top_n: int = 10) -> list[str]:
    ...
    sorted_counties = sorted(
        ((county, count) for county, count in total.items() if county != "其他"),
        key=lambda x: x[1],
        reverse=True,
    )
...
def plot_heatmap(all_years: dict[int, dict], top_counties: list[str], output_path: Path) -> None:
    ...
    matrix = np.array([[all_years[y].get(c, 0) for y in years] for c in top_counties], dtype=float)

    fig, ax = plt.subplots(figsize=(10, 7))
    im = ax.imshow(matrix, cmap="YlOrRd", aspect="auto")
    ax.set_xticks(range(len(years)))
    ax.set_xticklabels([f"{y} 學年" for y in years], fontsize=10)
    ax.set_yticks(range(len(top_counties)))
    ax.set_yticklabels(top_counties, fontsize=10)
    ...
    plt.colorbar(im, ax=ax, label="招生人數", shrink=0.82)
    ax.set_title("109-114 學年度來源縣市招生熱力圖（前 10 名縣市）", fontweight="bold")
    ax.set_xlabel("學年度")
    ax.set_ylabel("來源縣市")
