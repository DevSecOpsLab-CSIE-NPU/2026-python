...
ZIPCODE_TO_COUNTY = {
    ...
    "310": "新竹縣", "350": "苗栗縣", "351": "苗栗縣", "360": "苗栗縣",
...
def get_top_counties(all_years: dict[int, dict], top_n: int = TOP_N) -> list[str]:
    totals = {}
    for y, counties in all_years.items():
        for c, v in counties.items():
            totals[c] = totals.get(c, 0) + v
    sorted_counties = sorted(totals.items(), key=lambda x: -x[1])
    return [c for c, _ in sorted_counties[:top_n]]


def main():
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    year_data = {y: load_county_counts(y) for y in YEARS}
    top_counties = get_top_counties(year_data)

    matrix = np.array([[year_data[y].get(c, 0) for y in YEARS] for c in top_counties])
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "task2.png")
    print(f"圖表已儲存：{OUTPUT_DIR / 'task2.png'}")
    plt.close(fig)
