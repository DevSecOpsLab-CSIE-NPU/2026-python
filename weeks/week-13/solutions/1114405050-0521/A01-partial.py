...
# 使用 partial 將 exp 參數固定，分別創造出「平方」與「立方」的新函數
...
# 利用 partial 產生專門用來取「數學」和「英文」成績的函數，作為排序的依據 (key)
...
def cost_in_base(n, base, costs):
    ...
    if n == 0:
        return costs[0]
    total = 0
    # 透過不斷除以 base 取餘數，取得每一個進位位數的數字，並累加該數字對應的成本
    while n > 0:
        total += costs[n % base]
        n //= base
    return total
...
# 這樣在後續的列表推導式中，呼叫 calc 時就不必每次都重複傳入 uniform_costs
...
# 把 print 的結尾符號 (end) 從預設的換行 ('\n') 固定為空白 (' ')，產生一個不換行的 print 函數
...
# lambda 寫法：建立一個匿名函數來包裝原本的函數調用
double_lambda  = lambda x: power(x, 2)        # lambda 寫法
# partial 寫法：直接宣告要固定哪個參數，意圖更明顯，也較不容易踩到閉包延遲綁定的陷阱
