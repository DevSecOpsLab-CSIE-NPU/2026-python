...
def get_top_counties(all_years: dict[int, dict], top_n: int = 10) -> list[str]:
    ...
    for year_data in all_years.values():
        for county, count in year_data.items():
            total_counts[county] = total_counts.get(county, 0) + count
...
if __name__ == "__main__":
    ...
    # Plot heatmap
    heatmap_data = np.zeros((len(top_counties), len(years)))
    for i, county in enumerate(top_counties):
        for j, year in enumerate(years):
            heatmap_data[i, j] = all_years[year].get(county, 0)
    ...
    ax.set_title("109-114學年度前10大來源縣市新生人數熱力圖")
    fig.tight_layout()
    plt.savefig(output_dir / "task2.png", dpi=300)
    plt.close()
