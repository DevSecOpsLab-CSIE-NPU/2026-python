...
"""
11332 hand-written 版本

此檔為教學版，說明極角掃描與 active set 的實作細節，並保留較多註解以利學習。
"""
...
def solve_case(segments: list[Segment]) -> list[int]:
    # 先把每一個線段轉成極角區間，再用掃描線找在每個角度最靠近原點的線段。
    # 只要知道某段角度區間內，哪一條線段離原點最近，就能判斷它是否可見。
    ...
    angles = sorted(set(events) | {0.0, 2 * math.pi})
    ...
    for i in range(len(angles) - 1):
        ...
        if next_angle - angle < 1e-12:
            continue

        # 取兩個事件角度的中間值，避免剛好踩在邊界上造成判定不穩定。
        ...
        for kind, seg_id in sorted(events.get(angle, [])):
            if kind == -1:
                root = erase(root, id_to_seg[seg_id])
        for kind, seg_id in sorted(events.get(angle, [])):
            if kind == 1:
                root = insert(root, id_to_seg[seg_id])

        # 當前角度下，active set 裡最靠近原點的線段就是目前可見的那一條。
        best = get_min(root)
        if best is not None:
