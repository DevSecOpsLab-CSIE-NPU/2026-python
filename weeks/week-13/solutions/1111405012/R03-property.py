...
class Student:
    """用 property 保護 grade 範圍的學生類別。"""

    def __init__(self, name, grade):
        self.name = name
        self.grade = grade   # 這裡刻意走 setter，初始化時也會做驗證
...
class GradStudent(Student):
    """覆寫父類 setter，放寬研究生成績上限。"""

    @Student.grade.setter
    def grade(self, value):
        # 子類別沿用同一個屬性名稱，但把驗證規則改成自己的版本。
        if not (0 <= value <= 150):
