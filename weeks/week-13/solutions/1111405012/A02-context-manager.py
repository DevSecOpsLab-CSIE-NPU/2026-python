...
class Timer:
    ...
    def __enter__(self):
        # 進入 with 的第一瞬間先記錄起始時間。
        self.start = time.time()
        print("⏱  開始計時")
        return self   # 這個值會被 as 接收，例如 as t

    def __exit__(self, exc_type, exc_val, exc_tb):
        # 不管 with 區塊內成功或失敗，這裡都一定會執行。
...
def section(title):
    ...
    # yield 後面的部分，相當於 class 版 __exit__ 的工作。
    print(f"{'─'*40}")
...
def capture_output():
    """暫時把 print 的輸出截取到字串裡。"""
    old_stdout = sys.stdout                 # 先保存原本的標準輸出
    sys.stdout = buffer = io.StringIO()    # 改接到記憶體中的字串緩衝區
    try:
...
def solve_parity(n):
    ...
    bits = bin(n)[2:]          # bin(10) 會得到 '0b1010'，所以把前綴 0b 去掉
    ones = bits.count('1')     # 直接統計字串裡有幾個字元 '1'
...
captured = out.getvalue()   # 從 StringIO 把剛剛累積的輸出整包讀出來
print("截取到的輸出：")
print(captured)
