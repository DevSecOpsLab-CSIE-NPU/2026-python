...
from functools import partial   # partial 會回傳「先綁定部分參數」的新函數

# ── 基本概念：固定部分參數，產生新函數 ───────────────────

def power(base, exp):
    """回傳 base 的 exp 次方。"""
    # 先保留最簡單的函數本體，partial 的效果才會一眼看懂。
    return base ** exp

# square / cube 都不是新語法糖，本質上就是把 power 包裝成較專用的版本。
# 之後呼叫 square(5) 時，等同於 power(5, exp=2)。
...
def get_score(student, subject):
    """根據科目名稱，取出指定學生的成績。"""
    # sorted(key=...) 需要的是「輸入一筆資料，回傳排序依據」的函數。
    return student[subject]

# 把「依哪一科排序」先固定起來，sorted 之後就能直接拿來用。
...
DIGITS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"   # 方便對照 0-9 與 A-Z 的索引位置

def cost_in_base(n, base, costs):
    """計算 n 在 base 進位下每一位數字的成本總和。"""
    if n == 0:
        return costs[0]   # 數字 0 在任何進位都只會寫成單一位的 0

    total = 0
    while n > 0:
        digit = n % base      # 先取出目前最低位的數字值
        total += costs[digit] # 查成本表，把這一位的成本加進去
...
# 用 partial 固定 costs，之後只要填 (n, base) 即可。
...
# 這裡把 print 的 end 先固定成空白，之後就能反覆用在同行輸出。
...
double_lambda  = lambda x: power(x, 2)         # lambda：直接把 2 寫死在匿名函數裡
double_partial = partial(power, exp=2)         # partial：明確表達「固定的是 exp」
