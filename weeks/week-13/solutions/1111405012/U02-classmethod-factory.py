...
class Point:
    ...
    def from_string(cls, s):
        """從 '3,4' 這種字串建立 Point。"""
        # cls 就是「目前的 class 本身」；若子類呼叫，cls 就會自動變成子類。
        x, y = map(int, s.split(','))   # split 後得到 ['3', '4']，再轉成整數
        return cls(x, y)

    @classmethod
    def from_list(cls, lst):
        """從 [3, 4] 這種 list 建立 Point。"""
        # 把資料解析責任留在工廠方法，__init__ 就能專心處理欄位存放。
...
class CostTable:
    ...
    def total_cost(self, n, base):
        ...
        if n == 0:
            return self.costs[0]   # 數字 0 只有一位，所以直接回傳 0 的成本
        total = 0
        while n > 0:
            digit = n % base         # 先取目前最低位的 digit 值
            total += self.costs[digit]
            n //= base               # 去掉已處理過的最低位
        return total
