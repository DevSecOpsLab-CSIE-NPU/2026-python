...
ZIPCODE_TO_COUNTY = {
    ...
    "958": "台東縣", "959": "台東縣", "961": "台東縣", "962": "台東縣",
...
def zip_to_county(zipcode: str) -> str:
    ...
    text = (zipcode or "").strip()
    if len(text) < 3:
        return "其他"
    return ZIPCODE_TO_COUNTY.get(text[:3], "其他")
...
def plot_task2(all_years: dict[int, dict[str, int]], output_dir: Path = OUTPUT_DIR) -> Path:
    ...
    fig_height = max(6, len(counties) * 0.48)
    ...
    ax.set_title("109-114 學年度各縣市招生人數熱力圖")
...
def main() -> None:
    ...
    output_path = plot_task2(all_years)
