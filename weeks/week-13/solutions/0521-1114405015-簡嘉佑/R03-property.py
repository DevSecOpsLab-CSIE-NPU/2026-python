...
#
# 沒有 @property 時，外部程式碼可以把任何值直接塞進屬性，無法攔截。
# 用 @property + setter 就能在賦值當下做驗證，讓類別更安全。
...
class Circle:
    ...
    @property
    def # 沒有對應的 @area.setter → 唯讀屬性
        # 嘗試 c.area = 100 會直接拋 AttributeError
        area(self):
        """面積是計算出來的，不該被直接設定"""
        import math
...
class GradStudent(Student):
    ...
    @Stu# 覆寫父類別的 setter，研究生成績上限放寬到 150
        # 注意：必須使用 Student.grade.setter 這個裝飾器，才能正確覆寫
        dent.grade.setter
    def grade(self, value):
        if not (0 <= value <= 150):
