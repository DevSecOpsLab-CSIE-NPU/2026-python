...
#
# 當同一個類別的物件需要從不同格式（字串、字典、CSV 行…）建立時，
# 若全部塞進 __init__ 會讓初始化方法變得複雜難讀。
# @classmethod 讓每種格式各自有一個清楚命名的「工廠方法」。
...
class Point:
    ...
    def from_string(cls, s):
        ...
        # 使用 cls(...) 而非 Point(...) 的好處：繼承時子類會用子類建立物件
        x, y = map(int, s.split(','))
        return cls(x, y)
...
# from_string 繼承自 Point，但 cls 會指向「實際呼叫的 class」ColoredPoint
# 因此 cls(x, y) = ColoredPoint(x, y)，不是 Point(x, y)
...
class CostTable:
    ...
    def total_cost(self, n, base):
        """計算數字 n 在 base 進位下的總印刷   # 0 只有一位數字
        total = 0
        while n > 0:
            total += self.costs[n % base]   # 最低位的字元成本
            n //= base
        return total
    ...
    def uniform(cls, cost=1):
        """建立所有字元成本相同的表（方便測試）"""
        # cls([cost] * 36) = CostTable([1, 1, 1, ...])
        return cls([cost] * 36)

    @classmethod
    def from_flat_string(cls, s):
        """從一行 36 個整數（空白分隔）建立成本表"""
        # 題目輸入格式通常就是一行數字，用此方法直接解析
    def from_flat_string(cls, s):
        """從一行 36 個整數（空白分隔）建立成本表"""
