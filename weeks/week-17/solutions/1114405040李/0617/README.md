...
# 0617 Starter - timeit + search

## Completion Notes

Implemented files:

- `test_timing.py`: unittest coverage for the `timeit` decorator.
- `timing.py`: `timeit` decorator with `repeat`, `records`, `last_elapsed`, no printing, and `ValueError` for `repeat < 1`.
- `search.py`: linear search and binary search. `binary_search` assumes sorted ascending input and does not mutate the list.
- `AI_LOG.md` and `TEST_LOG.md`: required process notes.
...
- `target = 99999`
- `repeat = 5`
- Data was already sorted with `list(range(n))`.

Observed result:

- `linear_search` returned `99999`.
- `binary_search` returned `99999`.
- `linear_search` average time: about `0.0026702` seconds.
- `binary_search` average time: about `0.00000242` seconds.
- `binary_search` was much faster because it repeatedly halves the remaining search range.

Analysis:

1. Linear search is simple and works on any list order, but in the worst case it checks every item, so its time complexity is `O(n)`.
2. Binary search requires sorted data. If the list is already sorted, search is `O(log n)` and very fast. If the program must sort first, the total cost becomes `O(n log n) + O(log n)`, so it is most useful when the data is already sorted or when many searches reuse the same sorted data.
