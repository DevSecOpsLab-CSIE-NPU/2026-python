...
使用 `timeit`（repeat=5）對已排序資料進行 linear_search vs binary_search 比較：

| 資料量 n | linear (s) | binary (s) | 加速比 |
|---------|-----------|-----------|-------|
| 100     | 0.000002  | 0.000002  | 1.47x |
| 1,000   | 0.000027  | 0.000002  | 12.07x |
| 10,000  | 0.000294  | 0.000004  | 81.13x |
| 100,000 | 0.003101  | 0.000013  | 246.14x |

### 觀察

1. **linear_search 是 O(n)**：n 增為 10 倍，時間也約增 10 倍（100→1,000→10,000）。
2. **binary_search 是 O(log n)**：n 增為 10 倍，時間只增加約 2–3 倍，斜率極緩。
3. **排序成本 vs 搜尋次數**：若只搜一次且資料未排序，linear 不需排序成本反而可能較快；但若搜尋多次，排序一次後用 binary 大幅划算。
4. **交叉點**：n=100 時兩者幾乎無差，n≥1,000 後 binary 明顯勝出。
