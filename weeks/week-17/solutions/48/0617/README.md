...
|---:|-----------:|-----------:|------:|
| 1,000 | 0.00000229 | 0.00000077 | 2.97x |
| 5,000 | 0.00003940 | 0.00000105 | 37.60x |
| 10,000 | 0.00024349 | 0.00000105 | 230.79x |
| 50,000 | 0.00062580 | 0.00000101 | 619.60x |

### 直覺判斷

- **linear_search**：O(n)，資料量線性成長時耗時也線性成長
- **binary_search**：O(log n)，即使 n 從 1,000 增加到 50,000，耗時幾乎不變
- **排序划不划算**：如果只搜尋一次且資料未排序，linear 可直接使用不需排序成本；反之若需多次搜尋，先排序再用 binary 會大幅節省時間
