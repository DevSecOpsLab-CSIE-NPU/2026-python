...
| 維度 | 說明 |
|:----|:----|
| **搜尋速度** | 大數據量 ($n=100000$) 下的查詢效能 |
| **記憶體效率** | 演算法執行時額外消耗的空間（常數 vs 線性） |
| **免排序便利性** | 資料不需事先排序即可直接搜尋 |
| **建置低開銷** | 搜尋前不需前置處理（排序、轉 set 等） |
| **程式碼簡潔度** | 實作複雜度與維護成本 |

### 各演算法評分

| 方法 | 速度 | 記憶體 | 免排序 | 建置開銷 | 簡潔度 | 解讀 |
|:----|:---:|:-----:|:-----:|:-------:|:-----:|:----|
| `linear_search` | 1 | 5 | 5 | 5 | 3 | 用時間換便利，即插即用零成本 |
| `binary_search` | 4 | 5 | 1 | 2 | 3 | $O(\log n)$ 高效，但強烈依賴排序 |
| `set_search` | 4 | 1 | 5 | 1 | 4 | 空間換時間，建 set 成本是致命傷 |
| 內建 `in` | 2 | 5 | 5 | 5 | 5 | 日常首選，C 底層優化，簡潔滿分 |
| `bisect` | 5 | 5 | 1 | 2 | 5 | 效能天花板，速度與記憶體兼得 |

### 正規化方式

所有維度均以 **1–5 整數評分**，5 為最佳表現。速度維度以 Stage 3 實測數據為基準分等，其餘維度依演算法特性評比。

### 雷達圖解讀

- `bisect` 與 `builtin_in` 佔據面積最大，代表整體權衡最佳
- `set_search` 在記憶體與建置開銷兩個維度嚴重失分，呈現明顯凹陷
- `linear_search` 與 `binary_search` 互補——前者免前置但慢，後者快但需排序
