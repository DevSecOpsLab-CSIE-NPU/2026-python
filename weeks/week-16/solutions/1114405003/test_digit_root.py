...
class TestDigitRoot(unittest.TestCase):
    def test_basic(self):
        self.assertEqual(digit_root(199), 1)  # 199→19→10→1
        self.assertEqual(digit_root(38), 2)   # 38→11→2
        self.assertEqual(digit_root(7), 7)    # 一位數直接回傳

    def test_edge_case(self):
        self.assertEqual(digit_root(1), 1)    # 最小合法正整數
        self.assertEqual(digit_root(2000000000), 2)  # 大數邊界
        self.assertEqual(digit_root(10), 1)   # 剛好進位
