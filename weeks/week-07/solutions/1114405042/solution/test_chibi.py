...
# Add the directory to sys.path so we can import ChibiBattle
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from chibi_battle import ChibiBattle

# Path to the generals.txt file (parent directory of solution)
GENERALS_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'generals.txt')
...
class TestBattleLogic(unittest.TestCase):
    ...
    def test_defeated_generals(self):
        # Create an artificial scenario where a general is defeated
        self.game.calculate_damage('關羽', '郭嘉')
        self.game.calculate_damage('關羽', '郭嘉')
        self.game.calculate_damage('關羽', '郭嘉')
        self.game.calculate_damage('關羽', '郭嘉')
        self.game.calculate_damage('關羽', '郭嘉')
