...
# 本範例示範 timedelta 不支援月份、以及 strptime 與手動解析的效能差異。
...
# 正確做法：一個月不是固定天數，因此無法直接傳 months 參數給 timedelta
# 需要使用 calendar 取得目標月份天數，並把日期 clamp 到該月最後一天。
...
# ── strptime 效能問題（3.15）─────────────────────────
# datetime.strptime 會解析格式，若格式固定，可自行拆字串更快。
