...
def is_multiple_of_11(text):
    ...
    for index, char in enumerate(reversed(text)):
        digit = int(char)
        difference = difference + digit if index % 2 == 0 else difference - digit
    return difference % 11 == 0


def describe_number(text):
    return f"{text} is a multiple of 11." if is_multiple_of_11(text) else f"{text} is not a multiple of 11."
