...
# ═══════════════════════════════════════════════════════════
# 這一段示範三種資料格式的解析函式，
# 但若要比較速度，通常會在呼叫前後重複寫計時邏輯。
# 這樣程式碼不但冗長，還容易忘記改動。

def read_csv_raw(data: str) -> list:
    # 以 csv.DictReader 解析 CSV 字串資料，回傳每一列的 dict
    return list(csv.DictReader(io.StringIO(data)))


def read_json_raw(data: str) -> list:
    # json.loads 直接把 JSON 字串解析成 Python 物件
    return json.loads(data)


def read_xml_raw(data: str) -> list:
    # xml.etree.ElementTree 解析 XML，extract row 標籤的屬性
...
# 問題：這個 wrapper 會蓋掉原函式的 metadata
# 包含 __name__、__doc__、__module__ 等屬性。
...
def timeit(func):
    @functools.wraps(func)          # 保留 __name__ / __doc__ / __module__ 等資訊
    def wrapper(*args, **kwargs):
        start = time.perf_counter()
        result = func(*args, **kwargs)
        elapsed = time.perf_counter() - start
...
# ── 產生測試資料（1000 筆學生記錄）────────────────────────
# 先把三種格式的資料內容都準備好，後續再重複解析比較速度。
N = 1000

# CSV 格式：先寫入 StringIO，再取出字串
csv_buf = io.StringIO()
...
# ── 帶回傳耗時的計時包裝 ─────────────────────────────────
# timeit_silent 會保留執行結果，並且把耗時當作第二個回傳值。
