...
def solve(data: bytes) -> str:
    ...
    # 把 SCC 縮成 DAG。
    condensed_graph = [[] for _ in range(component_count)]
    ...
    for node in range(node_count):
        ...
        for neighbor in graph[node]:
            ...
            if source_component != target_component:
                condensed_graph[source_component].append(target_component)
    ...
    queue = deque(index for index in range(component_count) if indegree[index] == 0)
    while queue:
        component = queue.popleft()

        if dp[component] != -1:
            for neighbor in condensed_graph[component]:
                candidate = dp[component] + component_sum[neighbor]
