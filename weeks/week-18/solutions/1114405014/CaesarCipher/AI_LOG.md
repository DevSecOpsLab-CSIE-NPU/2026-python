...
> 第二題 Caesar Cipher 應該拆成哪些函式？核心函式與主程式如何分開？

AI 給了什麼：

> AI 建議設計 `caesar_cipher(text: str, shift: int) -> str` 作為核心函式，`process_text(input_text: str, shift: int) -> str` 處理多行輸入，`main()` 負責從 stdin 讀取輸入並輸出結果。

我改了什麼：

> 我採用函式拆分，讓核心加密邏輯可以被單元測試直接呼叫，而不是只能透過 stdin 測試。這也方便測試不同 shift 值，而不只測我的固定學號參數。
...
> 第二題輸入是否只有一行？要如何處理多行文字與 EOF？

AI 給了什麼：

> AI 說明題目輸入為多行文字，應讀到 EOF 結束。建議主程式使用 `sys.stdin.read()`，而不是只用一次 `input()`。

我改了什麼：

> 我確認不能只處理單行，因為題目要求多行輸入直到 EOF。因此我在主程式採用 `sys.stdin.read()`，確保多行、空行與換行都能被保留與處理。
...
> 凱撒密碼遇到非英文字元、空字串、空行時應該怎麼處理？

AI 給了什麼：

> AI 說明大寫英文字母在 `A-Z` 範圍循環，小寫英文字母在 `a-z` 範圍循環，非英文字元包含空白、數字、標點、換行都原樣保留。

我改了什麼：

> 我確認非英文字元不能被改動，因此在程式中保留 `else: result.append(char)`。同時要求測試涵蓋標點、空白、數字與空行，避免程式只對字母案例正確。
...
> AI 建議測試大寫位移、小寫位移、大寫循環、小寫循環、非字母保留、空字串、多行輸入、空行保留，以及不同 shift 值。

我改了什麼：

> 我保留不同 shift 值的測試，因為這能確認 `caesar_cipher()` 是真正依照參數運作，而不是只硬寫 `SHIFT = 5` 的結果。
...
> AI 說明完成標準包含：依學號計算 `SHIFT = 5`、大寫與小寫分開循環、非英文字元保留、多行輸入直到 EOF、pytest 全部通過。

我改了什麼：

> 我確認 `STUDENT_LAST_DIGIT = 4`，所以 `SHIFT = 4 % 25 + 1 = 5`。測試最初發生 `ModuleNotFoundError`，我判斷是檔名或位置問題，修正後 `10 passed`，符合驗收標準。
