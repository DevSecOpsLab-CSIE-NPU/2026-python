...
class Renderer:
    ...
    def draw_card(self, card, x, y, selected=False):
        ...
        if selected:
            pygame.draw.rect(self.screen, self.COLORS["selected"], rect, border_radius=5)
        else:
    ...
    def draw_last_play(self, cards, player_name, x, y):
        if cards:
            txt = self.small_font.render(f"{player_name}: ", True, (200, 200, 200))
            self.screen.blit(txt, (x, y))
            for i, c in enumerate(cards):
                self.draw_card(c, x + 80 + i * (self.CARD_WIDTH + 5), y)

    def draw_buttons(self, buttons, x, y):
    ...
    def draw_winner(self, player_name):
        txt = self.font.render(f"Winner: {player_name}!", True, (255, 215, 0))
        w = txt.get_width()
        self.screen.blit(txt, (400 - w // 2, 300))
