...
# 本檔把三種常見工程需求串在一起：
# 1) 壓縮文字/二進位資料（gzip）
# 2) 建立可自動清理的暫存工作區（tempfile）
# 3) 保存 Python 物件快照（pickle）
...
# 也能用 'wb'/'rb' 處理二進位資料。
# 這時候就不使用 encoding，因為我們處理的是原始 bytes。
...
with tempfile.TemporaryDirectory() as tmp:
    # TemporaryDirectory 回傳字串路徑，轉成 Path 比較好操作。
    tmp = Path(tmp)
    print("暫存資料夾:", tmp)
...
# delete=False 代表離開 with 不自動刪，讓你可以在外面再操作一次這個檔案。
with tempfile.NamedTemporaryFile("wt", delete=False, suffix=".log",
                                 encoding="utf-8") as f:
    f.write("暫存 log\n")
    log_path = f.name
print("暫存檔位置:", log_path)
Path(log_path).unlink()  # 用完自己刪，避免垃圾檔殘留

# ── 5.21 pickle：把 Python 物件「原樣」存檔 ────────────
# 適用：dict/list/tuple/自訂類別等 Python 物件
# 不適用：跨語言交換、長期保存（格式演進時建議用 json/parquet）
scores = {
    "alice": [90, 85, 92],
    "bob":   [70, 75, 80],
    "carol": [88, 91, 95],
