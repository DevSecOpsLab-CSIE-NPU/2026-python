...
# 'x' 模式會在檔案不存在時建立新檔，若檔案已存在則拋出 FileExistsError。
# 這很適合「每天只寫一次」的日記工具，不會覆蓋既有內容。
...
# ── 任務二：統計某資料夾裡 .py 檔的行數 ────────────────
# 走訪目錄 -> 逐檔讀取 -> 累計總行數、非空白行、def 函式定義行數。
def count_py(folder: Path):
    total, nonblank, defs = 0, 0, 0
    for p in folder.rglob("*.py"):
        # errors='replace' 可避免遇到亂碼時程式中斷
        with open(p, "rt", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if s:
                    nonblank += 1          # 非空白行
                if s.startswith("def "):
                    defs += 1              # 以 def 開頭的函式定義行
    return total, nonblank, defs

# 這裡示範讀取相對路徑的目錄，可依需求改成其他目標資料夾
